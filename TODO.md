...
* +Добавить признак для роли Reader
    ...
    rails generate migration add_active_tag_to_users writer:boolean
...
* Убрать отображение других пользователей для роли User
...
* +Содержимое для static_pages#home поместить в профиль http://localhost:3000/users/1

* +Добавить форму списка заявок для user
* +Для роли писатель добавить фильтры в контроллер orders index
    +чтобы в список заявок для писателя попадали только его заявки

* +Добавить форму списка всех заявок
* +Для роли читатель добавить фильтры в контроллер orders index
    +чтобы в список заявок для читателя попадали все не закрытые заявки

* +Для роли читатель добавить действия в контроллер orders
    +all         чтобы в список заявок для читателя попадали все заявки
    +accepted    чтобы в список заявок для читателя попадали все принятые заявки
    +rejected    чтобы в список заявок для читателя попадали все отклоненные заявки

* +Для роли читатель добавить ссылки на заявки
    +all             Все
    +accepted		Принятые
...
* Добавить в форму списка всех заявок параметры и фильтры

* Добавить запрет редактирования пользователем закрытых заявок

* Убрать отображение заявок других пользователей для роли User
...
* +Добавить признак Active для активации пользователя администратором
        rails generate migration add_active_tag_to_users active:boolean
* Добавить сообщение при авторизации, что пользователь не активирован администратором
...
* +Убрать строки "Find me" для режимов кроме dev
    <% if find_me? %>
